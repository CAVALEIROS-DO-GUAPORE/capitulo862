...
const TEMPLATE_BASE = 'relatorio_secretaria';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [mestre, escrivao] = await Promise.all([
    ...
    const buffer = fillTemplateDocxData(templatePath, {
      nome_mestre: mestre.data?.name ?? '',
      nome_escrivao: escrivao.data?.name ?? '',
    });
    ...
    console.error('[GET /api/downloads/relatorio-secretaria]', err);
    return NextResponse.json(
